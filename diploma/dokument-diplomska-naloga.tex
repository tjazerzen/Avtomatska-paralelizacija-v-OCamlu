...
Torej je $t_{par_{opt}} = \frac{t_{seq}}{n}$, kjer
$t_{par_{opt}}$ predstavlja optimalni čas izvajanja paralelnega programa, $t_{seq}$ čas izvajanja sekvenčnega programa ter $n$ število procesorjev.
...
posodobila razdaljo, preden bi druga nit imela priložnost posodobiti svojo razdaljo. To bi lahko privedlo do napačne razdalje
...
Da bi rešili problem dirke podatkov v paralelni implementaciji Dijkstrovega algoritma, smo se odločili za uporabo zaklepalnikov (ang. mutex).
Zaklepalniki so mehanizem, ki poskrbi za sinhronizacijo med več procesorskimi niti ter s tem prepreči dirko podatkov. V našem primeru
smo zaklepalnike uporabili za sinhronizacijo dostopa do vrste s prednostjo, ki je skupni pomnilnik.
...
Tu smo z \texttt{\dots} označili dele kode, ki so enaki kot v sekvenčni implementaciji. Dodatno smo dodali zaklepalnik \texttt{mutex},
ki ga ob vsakem dostopu do vrste s prednostjo zaklenemo, nato pa odklenemo.
...
Analiza je razkrila, da je paralelni Dijkstrov algoritem brez uporabe zaklepalnikov hitrejši od sekvenčne verzije algoritma,
sekvenčni algoritem pa je hitrejši od paralelnega algoritma z uporabo zaklepalnikov. Tak rezultat bi lahko razložili
z dodatnimi stroški, ki nastanejo pri uporabi zaklepalnikov, saj se zaradi njih poveča čas izvajanja algoritma.
...
Žal je paralelna implementacija brez uporabe zaklepalnikov dala napačne rezultate, zato je v tem primeru sekvenčna implementacija
najboljša izbira.
...
pridobljeno prioritetno vrsto sekvenčno združimo z obstoječo. Vseeno pa bi bil v tem primeru velik izziv tako grajenje nove prioritetne vrste kot učinkovito združevanje
z obstoječo prioritetno vrsto (posebej zaradi nespremenljivosti naše vrste s prednostjo).
