...
\kljucnebesede{paralelizacija\sep grafovski algoritmi\sep funkcijski programski jeziki\sep OCaml\sep iskanje v širino (BFS)\sep Dijkstrov algoritem\sep Floyd-Warshallov algoritem\sep Domainslib\sep nespremenljive podatkovne strukture\sep večjedrno računanje\sep optimizacija algoritmov\sep dinamično programiranje\sep vrsta s prednostjo\sep računska učinkovitost\sep višjenivojske abstrakcije}

% - angleški prevod ključnih besed
\keywords{parallelisation\sep graph algorithms\sep functional programming languages\sep OCaml\sep breadth-first search (BFS)\sep Dijkstra's algorithm\sep Floyd-Warshall algorithm\sep Domainslib\sep immutable data structures\sep multi-core computing\sep algorithm optimization\sep dynamic programming\sep priority queue\sep computational efficiency\sep high-level abstractions}
...
Nasprotno, \emph{imperativni programski jezik} je vrsta programskega jezika, kjer programer navede zaporedje ukazov,
