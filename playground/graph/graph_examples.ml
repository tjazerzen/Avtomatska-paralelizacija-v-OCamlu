...
let my_graph =
  ...
  |> UnweightedGraph.add_node node1
  |> UnweightedGraph.add_node node2
  |> UnweightedGraph.add_node node3
  |> UnweightedGraph.add_edge node1 node2
  |> UnweightedGraph.add_edge node2 node3
...
let () =
  print_endline
    "Creating weighted graph with nodes: {node1, node2, node3} and creating \
     edges node1-node2 (weight 0.3), node2-node3 (weight 0.4), and node1-node3 \
     (weight 0.5)..."
...
let my_weighted_graph =
  ...
  |> WeightedGraph.add_edge node1 node2 0.3
  |> WeightedGraph.add_edge node2 node3 0.4
  |> WeightedGraph.add_edge node1 node3 0.5

let () = print_endline "Printing weighted graph to string..."

let () =
  Printf.printf "WeightedGraph: %s\n"
