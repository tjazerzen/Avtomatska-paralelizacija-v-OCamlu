module Node : sig
  ...
  let to_string node = Printf.sprintf "Node(id: %d, value: %d)" node.id node.value
end
...
module Graph : sig
  ...
  let remove_node node graph =
    let edges =
      graph.edges
      |> NodeMap.remove node
      |> NodeMap.map (NodeSet.remove node)
    in
    { graph with edges }
  ...
  let remove_edge node1 node2 graph =
    ...
    let edges =
      if graph.directed then edges
      else edges |> NodeMap.add node2 (NodeSet.remove node1 (NodeMap.find node2 graph.edges))
    in
    { graph with edges } 
