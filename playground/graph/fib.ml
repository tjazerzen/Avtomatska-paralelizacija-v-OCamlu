...
module FibNumbersMixin : sig
  ...
  let par_fib_calculation_time ~num_domains n =
    ...
    let start = Unix.gettimeofday () in
    T.run pool (fun () -> let _ = FibNumbers.fib_par pool n in ());
    let stop = Unix.gettimeofday () in
    T.teardown_pool pool;
    stop -. start
  
  let par_calculation_time_num_domains_to_csv ~max_domains n =
    let out_channel = open_out "computation_time_analysis/fib_par_domains.csv" in
    let rec par_calculation_time_num_domains_to_csv_aux num_domains n =
      if num_domains > max_domains then ()
      else
        let calculation_time = par_fib_calculation_time ~num_domains:num_domains n in
  ...
  let par_calculation_time_fib_number_to_csv ~num_domains ~min_n ~max_n =
    ...
    let rec par_calculation_time_fib_number_to_csv_aux n =
      if n > max_n then ()
        ...
        let parallel_calculation_time = par_fib_calculation_time ~num_domains:num_domains n in
        let non_parallel_calculation_time = fib_calculation_time n in
        let multiplyer_between_times = non_parallel_calculation_time /. parallel_calculation_time in
        Printf.fprintf out_channel "%d,%.3f,%.3f,%.3f\n" n parallel_calculation_time non_parallel_calculation_time multiplyer_between_times;
        par_calculation_time_fib_number_to_csv_aux (n + 1)
      in
    output_string out_channel "fib_number,parallel_time,non_parallel_time,multiplyer_between_times\n";
