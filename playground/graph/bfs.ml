...
module BfsAlgorithms : Bfs = struct
  let loop (start_node : Node.t) (mapper : Node.t list -> NodeSet.t list)
      (graph : UnweightedGraph.t) : NodeSet.t list =
    let rec loop_inner (visited : NodeSet.t) (stages : NodeSet.t list)
        (mapper : Node.t list -> NodeSet.t list) (graph : UnweightedGraph.t) :
        NodeSet.t list =
      match stages with
      | last_stage :: _ ->
          let all_neighbors =
...
module MakeBfsPerformanceAnalysis (Bfs : Bfs) : sig
  ...
  val bfs_calculation_time_combinations_to_csv : (int * int) list -> int -> unit
end = struct
  let bfs_par_calculation_time (graph : UnweightedGraph.t) (start_node : Node.t)
    ...
    let _ =
      T.run task_pool (fun () -> Bfs.parallel graph start_node task_pool)
    in
