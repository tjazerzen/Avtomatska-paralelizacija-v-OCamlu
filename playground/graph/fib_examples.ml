...
print_endline
  (string_of_float
     (Fib.FibNumbersMixin.par_fib_calculation_time
        ~num_domains:current_num_domains current_fib_number));
...
Fib.FibNumbersMixin.par_calculation_time_num_domains_to_csv
  ~max_domains:max_num_domains current_fib_number;

print_endline
  "Printing parallel calculation times for Fibonacci numbers from 38 to 45 to \
   csv with 8 domains... (this should take approx. 15 seconds))";
Fib.FibNumbersMixin.par_calculation_time_fib_number_to_csv
  ~num_domains:current_num_domains ~min_n:min_fib_number ~max_n:max_fib_number
